 ...
 * Company          : Dontonno
 * Company URI      : https://dontonno.com/
 */
$(document).ready(function(){
    ...
    function calculate_vlsm(){
        ...
        var subnets = input_hosts.split(",").map(x => parseInt(x)).sort(desc);
    ...
    function createCard(networks){
        ...
        networks.forEach((network, e) => {
            ...
            cardHtml+='<h5 class="card-title">'+ $('.host_name').eq(e).val() +'</h5>';
        ...
        $('#final_result').html(cardHtml);
    }
    
    $('#generate_host_input').on('click', function(e){
        e.preventDefault();
        $('.show_on_condition').hide();
        if(parseInt($('#lan_number').val()) > 0){
            $('.show_on_condition').show();
        }
        var final_html = '';
        for (let index = 0; index < parseInt($('#lan_number').val()); index++) {
            var op = '<div class="form-row">'+
            '<div class="col-6">'+
